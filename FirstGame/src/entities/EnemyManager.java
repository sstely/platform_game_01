...
public class EnemyManager {
    ...
    private void drawSharks(Graphics g, int xLvlOffset, int yLvlOffset) {
        for(Shark s : currentLevel.getSharks())
            if(s.isActive()) {
                g.drawImage(sharkArr[s.getState()][s.getAniIndex()], (int) s.getHitbox().x - xLvlOffset - SHARK_DRAWOFFSET_X + s.flipX(), (int) s.getHitbox().y - yLvlOffset - SHARK_DRAWOFFSET_Y + (int) s.getPushDrawOffset(), SHARK_WIDTH * s.flipW(), SHARK_HEIGHT, null);
...
//				s.drawAttackBox(g, xLvlOffset);
    ...
    private void drawPinkstars(Graphics g, int xLvlOffset, int yLvlOffset) {
        for (Pinkstar p : currentLevel.getPinkstars())
            if(p.isActive()) {
                g.drawImage(pinkstarArr[p.getState()][p.getAniIndex()], (int) p.getHitbox().x - xLvlOffset - PINKSTAR_DRAWOFFSET_X + p.flipX(), (int) p.getHitbox().y - yLvlOffset - PINKSTAR_DRAWOFFSET_Y + (int) p.getPushDrawOffset(), PINKSTAR_WIDTH * p.flipW(), PINKSTAR_HEIGHT, null);
//				p.drawHitbox(g, xLvlOffset);
    ...
    private void drawCrabs(Graphics g, int xLvlOffset, int yLvlOffset) {
        for(Crabby c : currentLevel.getCrabs())
            if(c.isActive()) {
                g.drawImage(crabbyArr[c.getState()][c.getAniIndex()], (int) c.getHitbox().x - xLvlOffset - CRABBY_DRAWOFFSET_X + c.flipX(), (int) c.getHitbox().y - yLvlOffset - CRABBY_DRAWOFFSET_Y + (int) c.getPushDrawOffset(), CRABBY_WIDTH * c.flipW(), CRABBY_HEIGHT, null);
