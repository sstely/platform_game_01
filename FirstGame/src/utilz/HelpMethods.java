...
public class HelpMethods {
    ...
    public static float GetPlayerYPosUnderRoofOrAboveFloor(Rectangle2D.Float hitbox, float airSpeed) {
        ...
        if(airSpeed > 0) {
            // (Down) Falling - touching floor
            currentTile = (int)((hitbox.y + 8 * Game.SCALE) / Game.TILES_SIZE);
            int tileYPos = currentTile * Game.TILES_SIZE;
            int yOffset = (int)(Game.TILES_SIZE - hitbox.height);
            return tileYPos + yOffset - 1;
        } else {
    ...
    public static boolean IsAllTilesClear(int xStart, int xEnd, int y, int[][] lvlData) {
        for(int i = 0; i < xEnd - xStart; i++)
            if (IsTileSolid(xStart + i, y, lvlData)) //if there is any SOLID TILE between the characters
                return false;
        return true;
    }
    ...
    public static boolean IsAllTilesWalkable(int xStart, int xEnd, int y, int[][] lvlData) {
        if(IsAllTilesClear(xStart, xEnd, y, lvlData))
            for(int i = 0; i < xEnd - xStart; i++) {
                if (!IsTileSolid(xStart + i, y + 1, lvlData)) //if there is any PIT between the characters
                    return false;
            }
        return true;
